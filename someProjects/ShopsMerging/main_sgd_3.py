...
todo 先将文本转化为ids (貌似对速度影响并不大，转化为ids以后还需要进行两个文件的读写；
                        不过貌似只需要转化一次;
                        ）
'''
...
class ShopCompare():
    ...
    def shuffle(self, data, labels):
        ...
        data,labels =  [data[ind] for ind in index], [labels[ind] for ind in index]
        ind = int(self._conf.dataSplitRatio * len(labels))
        self.trainData, self.trainLabels = data[:ind], labels[:ind]
