...
class ShopCompare():
    ...
    def forward(self):
        ...
        logits = tf.matmul(W, tf.reshape(self.vectorsHolder, (-1, self._conf.batchSize))) + b
    ...
    def addPlaceHolder(self):
        self.vectorsHolder = tf.placeholder(dtype=tf.float32,shape = (None,self._conf.hiddenSize),name = 'vectorsHolder')
        # self._conf.batchSize
    ...
    def runEpoch(self, newModel=False, verbose=True):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            if newModel:
                ...
                saver.restore(sess, self._conf.modelNmae)
            # for i in xrange(int(len(self.trainData) / self._conf.batchSize)):
            data, labels = self.generateBatch()
            inputVec = self.Composition(data)
            self.addPlaceHolder()
            logits = self.forward()
            ...
            for i in xrange(2):
                ...
                inputVec = self.Composition(data)
                print np.shape(inputVec),tf.shape(self.vectorsHolder)


                loss, _ = sess.run([loss, trainOp],feed_dict = {self.vectorsHolder:inputVec})
                # loss = sess.run(loss)
